...
class CheckoutProcessPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // Ambil order yang sedang berlangsung dari dummyOrders
    final ongoingOrder = dummyOrders.firstWhere(
      (order) => order['status'] == 'berlangsung',
      orElse: () => <String, Object>{},
    );
    final isPickup =
        ongoingOrder.isNotEmpty && ongoingOrder['delivery'] == false;
    // Use ongoingOrder for display if available, fallback to dummyCheckout
    final orderData = ongoingOrder.isNotEmpty ? ongoingOrder : dummyCheckout;
    final items = orderData.containsKey('orderSummary')
        ? List<Map<String, dynamic>>.from(orderData['orderSummary'] as List)
        : List<Map<String, dynamic>>.from(orderData['items'] as List);
    final restaurantName = orderData['restaurantName'] as String? ?? '';
    final locationDetail = isPickup
        ? (orderData['locationSeller'] ?? '')
        : (orderData['locationBuyer'] ?? '');

    // Proses status: 0 = konfirmasi resto, 1 = konfirmasi pembayaran, 2 = disiapkan, 3 = diantar
    ...
    final List<_StepProcess> steps = [
      ...
      _StepProcess(
        icon: isPickup ? '' : 'lib/assets/images/iconDelivery.png',
        label: isPickup ? 'Pick Up' : 'Makanan Dalam Perjalanan',
